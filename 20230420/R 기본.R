...
which(name == 'test2')
which(name != 'test2')
which(name == 'test5') # 결과값이 integer(0)이라고 되어있는 이유는 영어 o랑 숫자0 헷갈리지 말라고
...
total_score = midturm + final # 데이터타입은 벡터 = 벡터 + 벡터
total_score # 결과값 벡터

# 데이터프레임 생성 시 행렬이든 데이터프레임이든 벡터든 조건만 맞다면 데이터프레임으로 연결이 가능
...
# 데이터프레임에 파생변수를 생성 # 이것만 저장됨 그래서 프린터를 해야 결과가 나옴
...
rbind(students, new_student) # 오류나는 이유는 컬럼이 다름
